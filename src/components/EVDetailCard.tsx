...
export function EVDetailCard({ game, jackpot, ev, breakeven }: EVDetailCardProps) {
  ...
  return (
    <div className={`card ${ev.isPositiveEV ? 'glow-neon border-neon/20' : 'border-dark-600'}`}>
      <h2 className="text-xl font-bold mb-4">📐 Expected Value Analysis</h2>
      
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div>
          <div className="text-xs text-gray-500">Current Jackpot</div>
          <div className="text-xl font-bold text-gold">{formatCurrency(jackpot)}</div>
        </div>
        <div>
          <div className="text-xs text-gray-500">Lump Sum (est.)</div>
          <div className="text-lg font-bold text-gray-300">{formatCurrency(ev.lumpSum)}</div>
        </div>
        <div>
          <div className="text-xs text-gray-500">After Tax (est.)</div>
          <div className="text-lg font-bold text-gray-300">{formatCurrency(ev.jackpotAfterTax)}</div>
        </div>
        <div>
          <div className="text-xs text-gray-500">Net EV / Ticket</div>
          <div className={`text-xl font-bold ${ev.isPositiveEV ? 'text-neon' : 'text-danger'}`}>
      ...
      <div className="mb-4">
        <div className="flex justify-between text-sm mb-1">
          <span className="text-gray-400">Progress to +EV Breakeven</span>
      ...
      {/* Tax breakdown */}
      <div className="grid grid-cols-3 gap-4 p-3 bg-dark-700/50 rounded-lg text-sm">
        <div>
          <div className="text-xs text-gray-500">Federal Tax</div>
          <div className="text-gray-300">37%</div>
        </div>
        <div>
          <div className="text-xs text-gray-500">State Tax (TX)</div>
          <div className="text-neon">0% 🎉</div>
        </div>
        <div>
          <div className="text-xs text-gray-500">Lump Sum Factor</div>
